...
import { ClipboardList, Search, Shield, Users, X } from 'lucide-react';
...
export default function StaffModerationPanel({ viewer, onViewProfile }: StaffModerationPanelProps) {
  const [panel, setPanel] = useState<'directory' | 'audit' | null>(null);
  ...
  return (
    <div className="space-y-3" id="staff_moderation_panel">
      ...
      {panel && (
        <div className="border border-app rounded-2xl bg-inset/40 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-app bg-surface">
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-accent" />
              <h4 className="font-display font-bold text-sm text-app">
                {panel === 'directory' ? 'All neighbors' : 'Moderation audit log'}
              </h4>
            </div>
            <button
              type="button"
              onClick={() => setPanel(null)}
              className="p-1.5 rounded-full text-muted hover:text-app hover:bg-inset"
              aria-label="Close"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          {msg && <p className="px-4 py-2 text-xs font-semibold text-emerald-500">{msg}</p>}
          {err && <p className="px-4 py-2 text-xs font-semibold text-red-400">{err}</p>}

          {panel === 'directory' && (
            <div className="p-4 space-y-3 max-h-[min(28rem,55vh)] overflow-y-auto">
          ...
          {panel === 'audit' && (
            <div className="p-4 max-h-[min(28rem,55vh)] overflow-y-auto">
              {loading ? (
