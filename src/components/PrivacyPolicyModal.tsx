...
import { acceptPrivacy } from '../lib/privacyPolicyPrompt';
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="privacy_policy_title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between gap-3 p-5 border-b border-app shrink-0">
          <h4 id="privacy_policy_title" className="font-display font-bold text-app">
            {PRIVACY.shortTitle}
          </h4>
          {!required && onClose ? (
            <button
        ...
        <div className="p-5 border-t border-app space-y-4 shrink-0 bg-surface">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            />
            <span className="text-xs text-muted font-semibold leading-relaxed">{PRIVACY.acceptLabel}</span>
          </label>
          <button
            type="button"
            disabled={!checked}
          ...
          {required ? (
            <p className="text-[10px] text-subtle text-center font-semibold">{PRIVACY.viewAgainNote}</p>
          ) : null}
        </div>
      </div>
    </div>
