-- Awards seed data only (safe to re-run).
-- Run AFTER supabase-sql/awards.sql schema section, or if tables already exist.
-- Use this if the main migration failed partway through the INSERT block.
...
-- Backfill join ranks and grants if schema is already in place
WITH ranked AS (
